import {pgParams, pgQuery, pgCleanString} from './postgres-lib';
...
export const createResource = async (user, data) => {
  ...
  const name = data.name ? pgCleanString(data.name) : pgCleanString(data.address);
  const address = pgCleanString(data.address);
